...
export const login = async (req: Request, res: Response) => {
   try {
      ...
      if (previousToken) {
         ...
         return res.status(409).json({ error: `You are still logged in as ${fullName}, please log out to log in to another account` });
      }
...
export const resetPassword = async (req: Request, res: Response) => {
   try {
      ...
      if (!user.canResetPassword || !user.resetSessionExpiry || user.resetSessionExpiry < new Date()) {
