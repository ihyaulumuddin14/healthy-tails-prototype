import React, { useEffect, useState } from 'react'
import { openHours } from '../../../shared/constants/constant';
import MagneticButton from '../components/MagneticButton';

const Hours = () => {

   const [day, setDay] = useState('');
   const [seeAll, setSeeAll] = useState(false);

   useEffect(() => {
      const now = new Date();
      const dayNow = now.toLocaleDateString('en-US', { weekday: 'long' });
      
      setDay(dayNow);
   }, [])

   return ( 
      <section id='hours' className='w-full h-fit flex justify-center items-center '>
         <div className='w-full max-w-[1536px] h-full flex flex-col items-center py-20 px-[min(7vw,100px)]'>
            <h1 className='text-[clamp(2.5rem,5vw,4rem)] font-bold font-inter text-[var(--text-color)] leading-10 mb-5'>Paws Up, We're Open!</h1>
            <p className='text-lg font-inter text-[var(--text-color)] text-[clamp(1rem,1.5vw,2rem)] mb-20'>Tail wags and care — check our operating hours below.</p>

            {/* content */}
            <div className='w-full max-w-2xl h-fit flex flex-col items-end'>
               {openHours.filter(schedule => schedule.day === day).map((schedule, index) => (
                  HourList({ day: schedule.day, open: schedule.open, close: schedule.close, index, special: true })
               ))}

               {/* list open hours in a week */}
               <div className={`w-full h-fit grid ${seeAll ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}  ease-in-out duration-500 overflow-hidden mb-2`}>
                  <div className='w-full h-fit flex justify-center items-center overflow-hidden'>
                     <HoursList />
                  </div>
               </div>
               <button
                  onClick={() => setSeeAll(!seeAll)}
                  className='text-[var(--primary-color)] bg-[var(--secondary-color)] py-2 px-4 rounded-md cursor-pointer'>
                     {seeAll ? "See Less" : "See All"}
               </button>
            </div>

            <div className='w-full h-fit grid grid-cols-1 md:grid-cols-2 gap-5 mt-10'>
               <div className='w-ful h-fit flex flex-col gap-5'>
                  <h2 className='text-[clamp(2.5rem,5vw,4rem)] font-bold font-inter text-[var(--text-color)]'>We're Ready</h2>
                  <p className='text-lg font-inter text-[var(--text-color)]'>For vaccinations, sterilizations, and other time-sensitive procedures, we recommend booking in advance.</p>
               </div>

               <div className='w0-full h-fit flex flex-col gap-5 px-[min(7vw,100px)] py-10 justify-center'>
                  <MagneticButton type="fill" link={'/'}>
                     Schedule an Appointment
                  </MagneticButton>
                  <MagneticButton type="outline" link={'/'}>
                     Contact Us
                     <svg className="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path fill="currentColor" fillRule="evenodd" d="M12 4a8 8 0 0 0-6.895 12.06l.569.718-.697 2.359 2.32-.648.379.243A8 8 0 1 0 12 4ZM2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10a9.96 9.96 0 0 1-5.016-1.347l-4.948 1.382 1.426-4.829-.006-.007-.033-.055A9.958 9.958 0 0 1 2 12Z" clipRule="evenodd"/>
                        <path fill="currentColor" d="M16.735 13.492c-.038-.018-1.497-.736-1.756-.83a1.008 1.008 0 0 0-.34-.075c-.196 0-.362.098-.49.291-.146.217-.587.732-.723.886-.018.02-.042.045-.057.045-.013 0-.239-.093-.307-.123-1.564-.68-2.751-2.313-2.914-2.589-.023-.04-.024-.057-.024-.057.005-.021.058-.074.085-.101.08-.079.166-.182.249-.283l.117-.14c.121-.14.175-.25.237-.375l.033-.066a.68.68 0 0 0-.02-.64c-.034-.069-.65-1.555-.715-1.711-.158-.377-.366-.552-.655-.552-.027 0 0 0-.112.005-.137.005-.883.104-1.213.311-.35.22-.94.924-.94 2.16 0 1.112.705 2.162 1.008 2.561l.041.06c1.161 1.695 2.608 2.951 4.074 3.537 1.412.564 2.081.63 2.461.63.16 0 .288-.013.4-.024l.072-.007c.488-.043 1.56-.599 1.804-1.276.192-.534.243-1.117.115-1.329-.088-.144-.239-.216-.43-.308Z"/>
                     </svg>
                  </MagneticButton>
               </div>
            </div>
         </div>
      </section>
   )
}

export default Hours


const HoursList = () => {
   return (
      <ul className='w-full h-fit flex flex-col items-center'>
         {openHours.map((schedule, index) => (
            <li key={index} className='w-full h-fit'>
               <HourList day={schedule.day} open={schedule.open} close={schedule.close} index={index} />
            </li>
         ))}
      </ul>
   )
}

const HourList = ({ day, open, close, index, special = false }: {day: string, open: string, close: string, index:number, special?: boolean}) => {
   return (
      <div key={index} className={`w-full h-fit flex items-center justify-between rounded-lg p-5 ${special ? "border border-[var(--tertiary-color)]" : ""} ${index !== openHours.length - 1 ? "border-b border-[var(--tertiary-color)]" : ""}`}>
         <span className='flex items-center gap-2'>
            <p className='text-[clamp(1.5rem,3vw,2rem)] font-bold font-inter text-[var(--text-color)]'>{day}</p>
            {special && (
               <p className='py-2 px-3 text-[clamp(0.5rem,1.5vw,1rem)] font-inter text-slate-900 bg-[var(--tertiary-color)] rounded-2xl'>now</p>
            )}
         </span>
         <span className='text-[var(--text-color)] flex gap-2'>
            <time dateTime={open}>{open}</time>
            :
            <time dateTime={close}>{close}</time>
         </span>
      </div>
   )
}