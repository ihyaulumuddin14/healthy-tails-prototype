   ...
   return (
      <section className={`w-full h-[calc(100vh-128px)] grid ${isSidebarOpen ? "grid-cols-[300px_1fr]" : "grid-cols-[55px_1fr]"} transisiton-all duration-400 ease-in-out relative top-32 overflow-hidden gap-2`}>
         <aside className="w-full h-full bg-[var(--color-background)] flex flex-col items-end sticky top-0 rounded-r-4xl overflow-hidden">
            <header className="w-full max-w-[250px] h-fit relative flex justify-end px-4 py-5 border-b-2 border-border">
               <h1 className={`${isSidebarOpen ? "opacity-100" : "opacity-0"} transition-all duration-400 ease-in-out absolute top-1/2 -translate-y-1/2 left-4 pointer-events-none`}>{user?.name}</h1>
         ...
         <div className={`w-full h-full ${isSidebarOpen ? "blur-xl pointer-events-none md:blur-none md:pointer-events-auto" : ""} transition-all duration-400 ease-in-out rounded-l-4xl bg-[var(--color-background)] overflow-y-auto`}>
            {children}
         </div>
      </section>
   )
